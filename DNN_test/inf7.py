...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1.0,
        help="Time (seconds) to smoothly ramp in GRU residual after warmup (default: 1.0)",
    )
    parser.add_argument(
        "--residual-clip",
        type=str,
        default="0.08,0.15,0.05",
        help="Per-joint residual clip in N.m as 'j1,j2,j3' (default: 0.08,0.15,0.05)",
    )
    return parser.parse_args()


def parse_clip_values(clip_str: str) -> np.ndarray:
    parts = [p.strip() for p in clip_str.split(",") if p.strip() != ""]
    if len(parts) != 3:
        raise ValueError("--residual-clip must have exactly 3 comma-separated values, e.g. 0.08,0.15,0.05")
    clip_vals = np.array([float(v) for v in parts], dtype=np.float64)
    if np.any(clip_vals <= 0):
        raise ValueError("--residual-clip values must be positive")
    return clip_vals
...
# ============================================================================
args = parse_args()
residual_clip = parse_clip_values(args.residual_clip)
...
print(f"🧩 Residual ramp: {args.residual_ramp_sec:.3f}s | clip: {residual_clip.tolist()} N.m")
print("="*70)
...
# 1) Clip residual per joint to avoid extreme corrections.
tau_residual_clipped = np.clip(tau_residual, -residual_clip, residual_clip)

# 2) Smoothly ramp GRU correction after warmup to avoid a torque jump at SEQ_LEN-1.
start_idx = SEQ_LEN - 1
tail_len = len(final_torques) - start_idx

if tail_len > 0:
    if args.residual_ramp_sec <= 0:
        ramp = np.ones(tail_len, dtype=np.float64)
    else:
        dt = float(np.median(np.diff(t))) if len(t) > 1 else 0.01
        ramp_steps = max(1, int(np.round(args.residual_ramp_sec / dt)))
        ramp = np.ones(tail_len, dtype=np.float64)
        end = min(tail_len, ramp_steps)
        ramp[:end] = np.linspace(0.0, 1.0, end, dtype=np.float64)

    final_torques[start_idx:] += tau_residual_clipped * ramp[:, None]
