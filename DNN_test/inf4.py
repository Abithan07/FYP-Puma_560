...
# ============================================================================
CSV_INPUT_PATH   = "path_460_joint_states_1.csv"         # ← Your raw input data
CSV_OUTPUT_PATH  = "path_460_joint_states_1_pred.csv"  # ← Where to save the results
...
df_input = pd.read_csv(CSV_INPUT_PATH, header=None)  # Read without header

# Transpose from row format to column format
df = df_input.T
df.columns = ['t'] + Q_COLS + QD_COLS + QDD_COLS
print(f"   ↳ Data shape: {df.shape} (transposed from row format)")
...
# Create output dataframe in row format
output_data = {
    't': t,
    'q_0': q[:, 0],
    'q_1': q[:, 1],
    ...
    'tau_hybrid_j1': final_torques[:, 0],
    'tau_hybrid_j2': final_torques[:, 1],
    'tau_hybrid_j3': final_torques[:, 2],
}

output_df = pd.DataFrame(output_data)

# Transpose to row format and save
output_df_transposed = output_df.T
output_df_transposed.to_csv(CSV_OUTPUT_PATH, header=False, index=False)
...
plt.tight_layout()
plot_path = CSV_OUTPUT_PATH.replace('.csv', '_comparison_plots.png')
...
for joint_name, idx in joints:
    ...
    single_plot_path = CSV_OUTPUT_PATH.replace('.csv', f'_joint_{joint_name}_detail.png')
