#!/usr/bin/env python3
"""
Continuous Logger - Triggered Mode - 1DOF Version
...
Usage:
    python3 continuous_logger_triggered_1dof.py
...
class TriggeredLogger1DOF(Node):
    def __init__(self):
        super().__init__('triggered_logger_1dof')
        ...
        self.csv_filename = os.path.join(
            self.log_dir, 
            f'trajectory_log_1dof_{self.test_number}_{timestamp}.csv'
        )
        ...
        # Store latest joint state data (single joint)
        self.latest_position = 0.0
        self.latest_velocity = 0.0
        self.latest_effort = 0.0
        ...
        self.get_logger().info('TRIGGERED LOGGER 1DOF - Ready for Commands')
        self.get_logger().info('='*70)
    ...
    def _get_next_test_number(self):
        ...
        test_num = 1
        while True:
            pattern = f"trajectory_log_1dof_{test_num}_"
    ...
    def start_logging_callback(self, request, response):
        ...
        # Write header (all 3 joints, but only joint 1 has real data)
    ...
    def joint_state_callback(self, msg):
        """Store the latest joint state data for joint_1 only"""
        try:
            # Find index for joint_1
            idx1 = msg.name.index('joint_1')
            
            # Extract position
            self.latest_position = msg.position[idx1]
            
            # Extract velocity
            if len(msg.velocity) > idx1:
                self.latest_velocity = msg.velocity[idx1]
            
            # Extract effort (torque)
            if len(msg.effort) > idx1:
                self.latest_effort = msg.effort[idx1]
    ...
    def timer_callback(self):
        ...
        # Write data row (joint 1 with real data, joints 2 & 3 filled with -40)
        row = [
            f"{elapsed:.3f}",
            f"{self.latest_position:.6f}",
            "0.000000",  # pos2 placeholder
            "0.000000",  # pos3 placeholder
            f"{self.latest_velocity:.6f}",
            "0.000000",  # vel2 placeholder
            "0.000000",  # vel3 placeholder
            f"{self.latest_effort:.6f}",
            "0.000000",  # torque2 placeholder
        ...
        if self.sample_count % 100 == 0:
            ...
            self.get_logger().info(
                ...
                f'Pos: {self.latest_position:.3f} rad'
            )
